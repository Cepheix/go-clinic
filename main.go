...
func RegisterModules(router gin.IRouter, configuration common.Configuration) {
	...
	AddSwaggerMiddleware(api, configuration, apiPrefix)

	appointments.RegisterModule(api.Group("/appointments"))
}

func AddSwaggerMiddleware(router gin.IRouter, configuration common.Configuration, prefix string) {
	url := ginSwagger.URL(configuration.Server.SwaggerDocumentAddress(prefix)) // The url pointing to API definition
